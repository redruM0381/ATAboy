...
int32_t tud_msc_read10_cb(uint8_t lun, uint32_t lba, uint32_t offset,
    ...
    if (offset && remaining > 0 && cur_lba < max) {
        uint8_t temp[512];
        if (ide_read_sectors(cur_lba, 1, temp) < 0) return -1;
    ...
    if (aligned > 0 && cur_lba < max) {
        ...
        if (ide_read_sectors(cur_lba, aligned, ptr) < 0) return -1;
...
int32_t tud_msc_write10_cb(uint8_t lun, uint32_t lba, uint32_t offset,
    ...
    if (offset && remaining > 0 && cur_lba < max) {
        ...
        if (ide_write_sectors(cur_lba, 1, temp) < 0) return -1;
        ptr += n; remaining -= n; cur_lba++;
    }
    ...
    if (aligned > 0 && cur_lba < max) {
        if (cur_lba + aligned > max) aligned = (uint32_t)(max - cur_lba);
        if (ide_write_sectors(cur_lba, aligned, ptr) < 0) return -1;
        ptr += aligned * 512; remaining -= aligned * 512; cur_lba += aligned;
    }
