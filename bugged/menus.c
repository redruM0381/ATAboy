...
extern "C" {
#endif
    /* Only import the helper actually implemented in ide_logic that this file calls.
       All other core1-owned state must be accessed via ide_* helpers. */
...
/* Reintroduce UI flags / flash pointer used throughout this file */
bool show_detect_result = false;
int  confirm_type = 0;

/* Flash image pointer (provided by linker / flash mapping) */
extern const uint8_t flash_target_contents[];

/* Fallback weak definition if the linker-provided symbol is not present.
   Prevents undefined reference at link time while preserving intended behavior. */
__attribute__((weak, aligned(4)))
const uint8_t flash_target_contents[FLASH_PAGE_SIZE] = {0};
...
// comp_timings is core1-owned; use ide_get_core_timings()/ide_set_core_timings() instead of accessing directly.
// extern bool comp_timings; // The Global Variable for Compat Timings
...
void update_main_menu() {
    ...
    /* Use a consistent snapshot of core1 state instead of reading core1 globals directly */
    ide_core1_state_t st;
    (void) ide_get_core1_state(&st);

    if (st.use_lba_mode) {
        uint64_t total = st.total_lba;
        uint32_t mb_size = total ? (uint32_t)((total * 512ULL) / 1048576ULL) : 0;
        snprintf(geo_vals, sizeof(geo_vals), "LBA Mode Active (%lu MB)", (unsigned long)mb_size);
    } else {
        snprintf(geo_vals, sizeof(geo_vals), "%u Cyl / %u Hd / %u SPT",
                 (unsigned)st.cyls, (unsigned)st.heads, (unsigned)st.spt);
    }
    ...
    bool is_valid = (st.use_lba_mode && st.total_lba > 0) || 
                    (!st.use_lba_mode && st.cyls > 0 && st.heads > 0 && st.spt > 0);
...
void update_features_menu() {
    ...
    /* Take a snapshot of core1 state for display to avoid direct core1 globals access */
    ide_core1_state_t st;
    (void) ide_get_core1_state(&st);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (i == 0) printf("%-8s", st.drive_write_protected ? "Enabled" : "Disabled");
        else if (i == 1) printf("%-8s", auto_mount    ? "Enabled" : "Disabled");
        else if (i == 2) printf("%-8s", iordy_pin     ? "Pin 27" : "Software");
        else if (i == 3) printf("%-8s", ide_get_core_timings() ? "Compatible" : "Normal");
...
int get_input() {
    // Keep USB stack alive while waiting for input
    // We poll in small chunks to ensure TUSB is serviced frequently
    for(int i = 0; i < 100; i++) { // Approx 100ms timeout total
        tud_task();
...
void save_config() {
    config_t config = {0};
    config.magic = 0x1DE45701;
    config.main_selected = main_selected;
    config.feat_selected = feat_selected;
    // Drive write-protect is taken from a core1 snapshot below (avoids races).
    config.auto_mount = auto_mount;
    config.iordy_pin = iordy_pin;
    config.comp_timings = ide_get_core_timings();

    /* Read a consistent snapshot of Core1 geometry/state */
    ide_core1_state_t st;
    if (ide_get_core1_state(&st)) {
        config.drive_write_protected = st.drive_write_protected;
        config.cyls  = st.cyls;
        config.heads = st.heads;
        config.spt   = st.spt;
    } else {
        /* Fallback: assume not write-protected and zero geometry */
    ...
    // Flash writing is critical; pause other core if possible, or just lock
    // Since Core 1 is in a queue wait, it shouldn't interfere with Flash (XIP) 
    // unless it wakes up. Ideally we pause it, but standard lockout is safer.
...
void load_config() {
    ...
    if (config.magic == 0x1DE45701) {
        ...
        /* Apply flags via atomic setters to avoid Core0 writing core1-owned state */
        ide_set_drive_write_protected(config.drive_write_protected);        
        auto_mount = config.auto_mount;
        iordy_pin = config.iordy_pin;
        ide_set_core_timings(config.comp_timings);
        /* Apply geometry atomically to Core1 via IPC helper */
        ide_set_core_geometry(config.cyls, config.heads, config.spt);
    }
}

void load_defaults() {
    ide_set_drive_write_protected(true);
    auto_mount = false;
    iordy_pin = false;
    ide_set_core_timings(false);    
    main_selected = 0;
    feat_selected = 0;
    /* Clear core1 geometry via IPC helper to avoid multicore races */
    ide_clear_geometry();
}
...
void ui_loop_logic() {
    ...
    int k = get_input();
    if (k == -1) return; // No input, just loop
    ...
    if (show_detect_result) {
        if (k == KEY_ENTER || k == KEY_ESC) {
            ...
            hdd_status_text[0] = '\0';
            hdd_model_raw[0] = '\0';
            /* Use IPC-safe helpers instead of touching core1 globals directly */
            ide_clear_geometry();
            ide_set_total_lba(0);
            ide_set_lba_mode(false);
            ide_export_drive_geometry(0,0,0);
            ide_set_mounted(false);
            needs_full_redraw = true;
        }
        return; 
    }
    ...
    if (current_screen == SCREEN_DEBUG) {
        if (k == KEY_ESC) {
            ...
            double x = 0;
            while (true) {
                // Modified loop to keep USB alive
                tud_task();
                ...
                // Simple polling loop with USB support
                // Since ide_read_8 calls via proxy are blocking, we rely on proxy latency
                // which is fine for seek test visualization
                while (ide_read_8(7) & 0x80) { tud_task(); }
                
                if (ide_read_8(7) & 0x08) {
                    for (int i = 0; i < 256; i++) {
                         ide_read_8(0); // Dummy read
                    }
                }
    ...
    if (current_screen == SCREEN_CONFIRM) {
        if (k == 'y' || k == 'Y') {
            if (confirm_type == 0) { load_defaults(); save_config(); current_screen = SCREEN_MAIN; }
                ...
                ide_reset_drive();
                /* Use IPC-safe helpers instead of touching core1 internals directly */
                ide_clear_geometry();
                ide_set_total_lba(0);
                ide_export_drive_geometry(0,0,0);
                ide_set_mounted(false);
            ...
            else if (confirm_type == 3) { ide_set_mounted(true); ide_flush_cache(); ide_notify_media_changed(); current_screen = SCREEN_MOUNTED; } 
            else if (confirm_type == 4) { ide_set_mounted(false); ide_notify_media_changed(); current_screen = SCREEN_MAIN; }
            needs_full_redraw = true;
        } 
    ...
    if (current_screen == SCREEN_MAIN) {
        if (k == KEY_UP && main_selected % 3 > 0) main_selected--;
            ...
            if (main_selected == 0) { current_screen = SCREEN_FEATURES; feat_selected = 0; needs_full_redraw = true; } 
                ...
                ide_core1_state_t st;
                (void) ide_get_core1_state(&st);
                bool geometry_valid = (st.use_lba_mode && st.total_lba > 0) ||
                                      (!st.use_lba_mode && st.cyls > 0 && st.heads > 0 && st.spt > 0);
                        ...
                        while(waiting_for_choice) {
                            ...
                            if (choice == KEY_UP && geo_idx > 0) {
                                ...
                                for (int f = 0; f < 3; f++) {
                                    ...
                                    while(true) {
                                        // Pump USB task here too
                                        tud_task();
                                        int c = getchar_timeout_us(0);
                                        if (c == PICO_ERROR_TIMEOUT) { busy_wait_us(1000); continue; }
                            ...
                            else if (choice == KEY_ENTER) {
                                {
                                    ...
                                    uint16_t sel_cyls = 0;
                                    uint8_t sel_heads = 0, sel_spt = 0;
                                    uint64_t sel_lba = 0;

                                    if (geo_idx == 0) {
                                        ide_set_lba_mode(false);
                                        sel_cyls = id_buf[1];
                                        sel_heads = (uint8_t)id_buf[3];
                                        sel_spt = (uint8_t)id_buf[6];
                                        selection_valid = true;
                                    } else if (geo_idx == 1) {
                                        ide_set_lba_mode(false);
                                        get_large_geometry(id_buf[1], (uint8_t)id_buf[3], (uint8_t)id_buf[6], &sel_cyls, &sel_heads);
                                        sel_spt = (uint8_t)id_buf[6];
                                        selection_valid = true;
                                    } else if (geo_idx == 2 && drive_supports_lba) {
                                        ide_set_lba_mode(true);
                                        sel_cyls = id_buf[1];
                                        sel_heads = (uint8_t)id_buf[3];
                                        sel_spt = (uint8_t)id_buf[6];
                                        sel_lba = ((uint64_t)id_buf[103] << 48) | ((uint64_t)id_buf[102] << 32) |
                                                  ((uint64_t)id_buf[101] << 16) | ((uint64_t)id_buf[100]);
                                        if (sel_lba == 0) sel_lba = id_buf[60] | ((uint32_t)id_buf[61] << 16);
                                        selection_valid = true;
                                    } else if (geo_idx == 3 && detect_cyls > 0) {
                                        ide_set_lba_mode(false);
                                        sel_cyls = detect_cyls; sel_heads = detect_heads; sel_spt = detect_spt;
                                        selection_valid = true;
                                    }

                                    if (selection_valid) {
                                        ide_set_core_geometry(sel_cyls, sel_heads, sel_spt);
                                        /* Decide total-LBA from the user's selection (geo_idx==2 == LBA) — avoid reading core1's use_lba_mode directly. */
                                        if (geo_idx == 2) ide_set_total_lba(sel_lba);
                                        else ide_set_total_lba(0);

                                        if (ide_set_geometry(sel_heads, sel_spt)) {
                                            ide_export_drive_geometry(sel_cyls, sel_heads, sel_spt);
...
int main() {
    ...
    // START CORE 1 Worker (Dedicated IDE controller)
    ide_start_core1_worker();
    ...
    if (auto_mount) {
        ...
        if (st.cyls > 0 && st.heads > 0 && st.spt > 0) {
            ...
            if (st.use_lba_mode) {
                /* Export zeroed geometry atomically to core1-owned exports */
                ide_export_drive_geometry(0, 0, 0);
            } else {
                if (ide_set_geometry(st.heads, st.spt)) {
                    /* Export detected geometry atomically */
                    ide_export_drive_geometry(st.cyls, st.heads, st.spt);
                }
            }
        }
   }
    ...
    // MAIN LOOP (CORE 0)
    // Handles UI and USB simultaneously
    while (1) {
        tud_task();
        ui_loop_logic();
    }
